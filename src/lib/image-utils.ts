// Shimmer blur placeholder for images
export const shimmer = (w: number, h: number) => `
<svg width="${w}" height="${h}" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  <defs>
    <linearGradient id="g">
      <stop stop-color="#E8E6E1" offset="20%" />
      <stop stop-color="#F5F4F2" offset="50%" />
      <stop stop-color="#E8E6E1" offset="70%" />
    </linearGradient>
  </defs>
  <rect width="${w}" height="${h}" fill="#E8E6E1" />
  <rect id="r" width="${w}" height="${h}" fill="url(#g)" />
  <animate xlink:href="#r" attributeName="x" from="-${w}" to="${w}" dur="1s" repeatCount="indefinite"  />
</svg>`;

export const toBase64 = (str: string) =>
  typeof window === "undefined"
    ? Buffer.from(str).toString("base64")
    : window.btoa(str);

// Pre-generated blur placeholder (simple gray)
export const blurDataURL = `data:image/svg+xml;base64,${toBase64(shimmer(700, 475))}`;

// Small blur placeholder for avatars
export const avatarBlurDataURL = `data:image/svg+xml;base64,${toBase64(shimmer(100, 100))}`;
